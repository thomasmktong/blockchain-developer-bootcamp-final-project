...
contract StreakBank is Ownable, Pausable {
    ...
    /// @notice Calculates the total interest earned of the game.
    /// @return total interest earned
    function getTotalInterestEarned() public view returns (uint256) {
        return adaiToken.balanceOf(address(this)) - totalGamePrincipal;
    }
    ...
    function withdraw() external whenNotPaused {
        ...
        if (
            (player.mostRecentSegmentPaid + 1) >= currentSegment &&
            (player.mostRecentSegmentPaid - player.segmentJoined) >
            minSegmentForReward
        ) {
            // Player is eligible for interest
            playerReward =
                (getTotalInterestEarned() * player.amountPaid) /
                totalGamePrincipal;
            withdrawAmount += playerReward;
        }

        totalGamePrincipal -= withdrawAmount;
